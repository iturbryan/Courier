...
class Messages extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($this->input->post())){
		    ...
		    $messages = $this->Message->get_list();
		    
		    foreach($messages->result() as $message){
		    
		      $data = array('to_sender' => $this->get($message->id . '_sender'),
				    'to_receiver' => $this->get($message->id . '_receiver')
				    );
		    
		      $this->Message->update($message->id, $data);
		      
		    }
		    
		    $this->session->set_flashdata('success', 'Message Settingss successfully saved!');
		...
		$data['messages'] = $this->Message->get_list();
		
		$this->show('messages', $data);
	}
